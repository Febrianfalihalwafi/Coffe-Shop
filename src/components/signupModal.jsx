      ...
      <div className="bg-gradient-to-br from-[#7a5e3a] to-[#5d4a2f] px-5 py-4 text-center relative flex-shrink-0">
        {onClose && (
          <motion.button
            ...
            whileTap={{ scale: 0.9 }}
            // 🔽 UBAH: text-[#faf7f3]/90 → text-[#3d3226] (coklat gelap, bukan putih)
            className="absolute top-3 right-3 text-[#3d3226] hover:text-[#5d4a2f]"
          >
            <X size={16} />
          </motion.button>
        )}
        {/* 🔽 UBAH: text-[#ffffff]/95 → text-[#3d3226] (coklat gelap, bukan putih) */}
        <h2 className="text-base font-light text-[#ffffff] tracking-wide">
          Register Coffee Telkom
        </h2>
      </div>
        ...
        <AnimatePresence>
          {error && (
            <motion.div
              ...
              className="mb-4 px-3 py-2 bg-red-5aa0 border-l-4 border-red-500 rounded-r-lg text-xs text-red-700"
            >
              {error}
            </motion.div>
          )}
          ...
          {form.password && (
            <div className="flex gap-0.5 h-0.5">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className={`flex-1 rounded-full ${passwordStrength === "weak" && i === 1 ? "bg-red-400" : passwordStrength === "medium" && i <= 2 ? "bg-yellow-400" : passwordStrength === "strong" ? "bg-green-400" : "bg-[#e8dfd4]"}`}
                />
              ))}
            </div>
          )}
