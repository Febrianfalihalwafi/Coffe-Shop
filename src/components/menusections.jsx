...
const containerVariants = {
  ...
  visible: {
    opacity: 1,
    transition: { staggerChildren: 0.12, delayChildren: 0.15, ease: modernEase },
  },
...
const itemVariants = {
  ...
  visible: {
    ...
    transition: { duration: 0.6, ease: modernEase, type: "spring", stiffness: 280, damping: 24 },
  },
...
const modalContentVariants = {
  ...
  visible: {
    ...
    transition: { duration: 0.28, ease: modernEase, type: "spring", stiffness: 420, damping: 32 },
  },
  ...
  return (
    <section className="relative w-full py-20 md:py-28 bg-[#faf7f3] overflow-hidden">
      ...
      <div className="absolute inset-0 -z-10 pointer-events-none">
        ...
        {[...Array(4)].map((_, i) => (
          <motion.div
            ...
            style={{ left: `${12 + i * 20}%`, top: `${15 + (i % 2) * 35}%`, animationDelay: `${i * 0.6}s` }}
          >
            {i % 2 === 0 ? "☕" : "✦"}
          </motion.div>
        ))}
      ...
      <div className="relative z-10 text-center px-6 mb-10 md:mb-14">
        ...
        >
          Crafted <span className="font-semibold text-[#7a5e3a]">Excellence</span>
        </motion.h2>
        <motion.p
        ...
        >
          Every cup tells a story. Explore our curated collection of premium coffees.
        </motion.p>
      </div>
      ...
      {loading && (
        <div className="relative z-10 text-center py-16">
          ...
          <p className="text-sm text-[#6f5d48]">Memuat menu...</p>
        </div>
      )}
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {filtered.map((item) => (
              ...
              >
                {/* Image */}
                <div className="relative h-48 md:h-52 overflow-hidden bg-[#f0ebe4]">
                  {item.image ? (
                    <img
                      src={item.image}
                      alt={item.name}
                  ...
                  {item.features && (
                    <div className="flex flex-wrap gap-1 mb-3">
                      {item.features.slice(0, 2).map((feat, i) => (
                        <span key={i} className="text-[9px] px-2 py-0.5 bg-[#f5f0e8] text-[#6f5d48] rounded-full border border-[#e6e1da]/70">
                          {feat}
                        </span>
                      ))}
                    </div>
      ...
      <AnimatePresence>
        {selectedItem && (
          ...
          >
            ...
            >
              {/* Modal Image */}
              <div className="relative h-40 md:h-44 bg-[#f0ebe4]">
                {selectedItem.image ? (
                  <img
                  ...
                  <div className="w-full h-full flex items-center justify-center text-[#b5a898] text-2xl">
                    ☕
                  </div>
                )}
              ...
              <div className="p-4 md:p-5 space-y-4">
                ...
                {selectedItem.features && (
                  <div className="flex flex-wrap gap-1.5">
                    {selectedItem.features.map((feat, idx) => (
                      <span key={idx} className="text-[10px] px-2.5 py-1 bg-[#f5f0e8] text-[#6f5d48] rounded-full border border-[#e6e1da]">
                        {feat}
                      </span>
                    ))}
                  </div>
                ...
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {["Less Sugar", "Extra Shot", "Oat Milk"].map((opt) => (
                    <label key={opt} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-full bg-white/60 border border-[#e6e1da] text-[10px] text-[#4b3b2a] cursor-pointer hover:border-[#7a5e3a]/40 transition-all">
                      <input type="checkbox" className="accent-[#7a5e3a] w-3 h-3" />
                      {opt}
                    </label>
                  ))}
                </div>
                ...
                >
                  ...
                  Add to Cart — Rp{Number(selectedItem.price).toLocaleString("id-ID")}
                </motion.button>
